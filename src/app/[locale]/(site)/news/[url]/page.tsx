...
interface PageProps {
  params: Promise<LocaleRouteParams & { url: string }>;
}
...
export default async function NewsPage({ params }: PageProps) {
  ...
  const breadcrumbJsonLd = buildBreadcrumbList(locale, [
    { name: resolved.title, path: `/news/${url}` },
  ]);
        ...
        <div className="container mx-auto max-w-2xl px-4 py-16">
          <div className="rounded-xl border border-white/5 bg-white/[0.03] p-8 text-center md:p-10">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              {/*
                Deliberately leaves the current locale: this article only
                exists in English. `localizePath` with an explicit locale
                spells that as "/news/<slug>/" — the default locale has no
                prefix, and a literal "/en/…" would only be redirected.
              */}
              <Link
                href={localizePath(DEFAULT_LOCALE, `/news/${url}`)}
