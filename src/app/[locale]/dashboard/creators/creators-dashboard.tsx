...
interface Creator {
  ...
  /**
   * Discord user id used for the role sync, or `null` for creators added
   * before it existed. Dashboard-only — `/api/creators` never carries it.
   */
...
/**
 * State of the Discord role sync, as reported by the creators endpoint: whether
 * a bot is configured at all, which rank was marked as the creator rank, and a
 * ready-made note when no role can be handed out.
 */
...
const EMPTY_DRAFT: Draft = { name: "", minecraftUuid: "", discordId: "", channels: [] };
...
/**
 * Same idea for the Discord user id (`normalizeDiscordId` on the server). An
 * empty field is valid: a creator without a linked Discord account is a normal
 * creator, they simply receive no role.
 */
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="grid flex-1 gap-4 sm:grid-cols-2">
              ...
              <Field
                ...
                hint={
                  !discordIdValid
                    ? "17 bis 20 Ziffern, oder leer lassen."
                    : !discord.configured
                      ? "Rollensync inaktiv — es ist kein Discord-Bot eingerichtet."
                      : discord.notice
                        ? discord.notice
                        : discord.creatorRank
                          ? `Bekommt die Discord-Rolle des Rangs „${discord.creatorRank}“. Der Account muss auf dem OTP-Discord sein.`
                          : undefined
                }
              >
                <input
                  ...
                  className={`${inputClass} font-mono text-xs ${
                    !discordIdValid ? "border-red-500/40" : ""
                  }`}
                />
              </Field>
            </div>
...
function DeleteDialog({
  ...
  const remove = async () => {
    ...
    try {
      ...
      // 204 is the ordinary answer; a body only appears when the row is gone
      // but its Discord role could not be revoked, which the page then shows.
      const body = res.status === 204 ? {} : await res.json().catch(() => ({}));
      onDeleted(body.warning);
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
      setBusy(false);
    }
...
function CreatorRow({
  ...
  first: boolean;
  last: boolean;
  /** Level 2+: may reorder and edit. Below that the row is read-only. */
  canWrite: boolean;
  /** Level 3: may remove the creator. */
  canDelete: boolean;
  ...
  return (
    <tr className="border-b border-white/5 transition-colors last:border-0 hover:bg-white/[0.02]">
      <td className="py-3 pl-4 pr-2">
        {/* Reordering writes `sort_order` through the order route, so it is a
            write action and disappears below level 2. The cell stays, so the
            table keeps its columns. */}
        {canWrite && (
          <div className="flex flex-col">
            <button
              onClick={() => onMove(-1)}
...
function CreatorsDashboardContent() {
  // Level 1 sees the list only; level 2 adds creating, editing and reordering;
  // level 3 adds removing. The routes enforce the same — this only keeps the
  // buttons honest.
  const level = usePermissionLevel("creators");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setDiscord({
        configured: body.discord?.configured === true,
        creatorRank: body.discord?.creatorRank ?? null,
        notice: body.discord?.notice ?? null,
      });
    } catch (e) {
      setError(e instanceof Error ? e.message : String(e));
      setCreators([]);
   ...
   */
  const reorderable = canWrite && search.trim().length === 0;
      ...
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-white/40">
            ...
            {!discord.configured ? (
              <span className="text-white/25"> · Discord-Rollensync inaktiv</span>
            ) : discord.notice ? (
              <span className="text-white/25"> · kein Creator-Rang gesetzt</span>
            ) : discord.creatorRank ? (
              <span className="text-white/25">
                {" "}
                · Discord-Rolle: {discord.creatorRank}
              </span>
            ) : null}
          </p>
        </div>
                    ...
                    <CreatorRow
                      ...
                      onEdit={() =>
                        setDraft({
                          ...
                          // Creators from before the role sync have no id; the
                          // field stays empty and optional for them.
      ...
      {draft && (
        <CreatorModal
          ...
          onSaved={(warning) => {
            setDraft(null);
            load();
            // The record was written; only the Discord side had something to
            // report. It rides in the same notice box rather than a new one —
            // it is a problem the operator has to act on either way.
