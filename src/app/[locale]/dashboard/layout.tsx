...
/**
 * The sidebar entries. `area` names the dashboard area an entry belongs to and
 * is what decides whether it is shown at all — an entry whose area is at level
 * 0 is not rendered, because a link that can only ever end on "Access Denied"
 * is worse than no link. The overview has no area: it is reachable for anybody
 * who has a level somewhere, and shows only the tiles they may use.
 */
const navItems: {
  href: string;
  label: string;
  ...
  { href: "/dashboard/news", label: "News", icon: Newspaper, area: "news" },
  {
...
function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  // Fail-closed: an absent permission set reads as 0, so a session from before
  // this feature existed shows the overview only, never a link it may not use.
  const visibleNavItems = navItems.filter(
    (item) =>
      !item.area ||
      permissionLevel(session?.user?.permissions, item.area) > LEVEL_NONE,
  );
