...
/* --- Table row --- */
function NewsRow({ item, canWrite, canDelete, onEdit, onDelete }: { item: NewsItem; canWrite: boolean; canDelete: boolean; onEdit: (i: NewsItem) => void; onDelete: (i: NewsItem) => void }) {
...
function NewsDashboardContent() {
  // Level 1 gets the list and the "open article" links, nothing that writes;
  // level 2 additionally gets the editor; only level 3 gets the bin. The API
  // refuses all the same — hiding is guidance, not the boundary.
  const level = usePermissionLevel("news");
  ...
  // Belt and braces: the editor is only reachable through buttons that are
  // hidden below, but a level that drops while the editor is open must not
  // leave a form standing that can no longer be saved.
  if (view === "editor" && canWrite) {
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[880px]">
              ...
              <tbody>
                {filtered.map((item, i) => (
                  <NewsRow key={i} item={item} canWrite={canWrite} canDelete={canDelete} onEdit={(x) => { setEditing(x); setView("editor"); }} onDelete={setDeleteTarget} />
                ))}
              </tbody>
            </table>
          </div>
...
export default function NewsDashboard() {
  return <AuthGuard area="news"><NewsDashboardContent /></AuthGuard>;
}
