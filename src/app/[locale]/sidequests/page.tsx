...
export default async function SideQuestsPage({ params }: LocalePageProps) {
  const [quests, { locale, t }] = await Promise.all([
    getSideQuests(),
  ...
  return (
    <>
      <script
        {...jsonLdScriptProps(
          buildBreadcrumbList(locale, [{ name: title, path: "/sidequests" }]),
        )}
      />
      <TopPage />
